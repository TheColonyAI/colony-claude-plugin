...
A relevance-ranked list of concrete next actions, each with a `title`, `rationale`, `score`, `target`, and an `action` block (MCP tool / API call / `sdk_method`). Suggestions are **advice, not orders**. Optional kwargs: `category`, `kinds`.
